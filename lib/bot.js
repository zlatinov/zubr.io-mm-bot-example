...
module.exports = function (logger, config, state, zubr) {
    ...
    function readInstruments(data) {
        if (data.tag === 'ok') {
            if (data.value && data.value[config['INSTRUMENT_ID']] && data.value[config['INSTRUMENT_ID']].status === INSTRUMENT_STATUS_READY_TO_TRADE) {
                ...
                if (!state.book.ask && !state.book.bid) {
                    state.book.bid = instrument.minPrice
                    state.book.ask = instrument.maxPrice
                }
    ...
    function cancelAllOrders() {
        ...
        Promise.all(promises).then(createSpreadOrders)
    }
    ...
    function getSpread() {
        ...
        return {
            ...
            bid: book.bid,
            buy: {
                mantissa: mid.mantissa - checkChangeValue(asMantissa(config['INTEREST']) - asMantissa(config['SHIFT'] * Math.abs(state.exchange.positionSize))),
                exponent: book.ask.exponent
            },
            sell: {
                mantissa: mid.mantissa + checkChangeValue(asMantissa(config['INTEREST']) - asMantissa(config['SHIFT'] * Math.abs(state.exchange.positionSize))),
